import { Project, Task, TaskList } from "@/lib/types";
import { createMockProject } from "@/services/mock/mock";
import {
  ChartNoAxesColumn,
  ListFilter,
  Plus,
  TableProperties,
  X,
} from "lucide-react";
import React, { createContext, use, useEffect, useRef, useState } from "react";
import TaskListComponent from "./TaskList/TaskList";
import { Fascinate_Inline } from "next/font/google";
import { toastSuccess } from "../toast/toaster";

type DNDContextType = {
  draggingId: string | null;
  hoverTaskId: string | null;
  hoverListId: string | null;
  dragType: "list" | "task" | null;
  handleDragStart: (id: string, type: "list" | "task" | null) => void;
  handleDragOver: (
    targetId: string | null,
    type: "list" | "task" | null
  ) => void;
  handleAddTask: (listId: string, name: string) => void;
  handleMarkComplete: (id: string) => void;
  handleDeleteList: (id: string) => void;
};

export const DNDContext = createContext<DNDContextType>({
  draggingId: null,
  hoverTaskId: null,
  hoverListId: null,
  dragType: null,
  handleDragStart: () => {},
  handleDragOver: () => {},
  handleAddTask: () => {},
  handleMarkComplete: () => {},
  handleDeleteList: () => {},
});

export default function TaskTable({ id }: { id: string }) {
  const [isAdding, setIsAdding] = useState(false);
  const [project, setProject] = useState<Project | null>();
  // const [tasklist, setTaskList] = useState<TaskList[]>(project?.list || []);
  const [dragType, setDragType] = useState<"list" | "task" | null>(null);
  const [draggingId, setDraggingId] = useState<string | null>(null);
  const [hoverTaskId, setHoverTaskId] = useState<string | null>(null);
  const [hoverListId, setHoverListId] = useState<string | null>(null);
  const [expand, setExpand] = useState(true);
  const inputRef = useRef<HTMLInputElement>(null);

  const handleDragStart = (id: string, type: "list" | "task" | null) => {
    setDraggingId(id);
    setDragType(type);
  };

  const handleAddList = () => {
    if (!inputRef.current || !inputRef.current.value) return;

    const newList: TaskList = {
      id: inputRef.current.value,
      name: inputRef.current.value,
      list: [],
    };

    const updatedList = [...(project?.list || []), newList];

    setProject({
      ...project,
      list: updatedList,
    } as Project);

    // setTaskList(updatedList);
    toastSuccess("New task list added");
  };

  const handleDeleteList = (id: string) => {
    const updatedList = project?.list.filter((tl) => tl.id !== id) || [];
    setProject({
      ...project,
      list: updatedList,
    } as Project);

    // setTaskList(updatedList);
  };

  const handleAddTask = (listId: string, name: string) => {
    if (!name) return;
    const newTask: Task = {
      id: name,
      name: name,
      theme: "",
      description: "",
      member: [],
      comment: [],
      state: false,
      createdAt: new Date(),
      due: "",
    };

    const updatedLists = project?.list.map((list) =>
      list.id === listId ? { ...list, list: [...list.list, newTask] } : list
    );

    setProject({ ...project, list: updatedLists } as Project);
    // setTaskList(updatedLists as TaskList[]);
  };

  const handleMarkComplete = (id: string) => {
    toastSuccess(id);
    const updatedLists = project?.list.map((list) => {
      const updatedTasks = list.list.map((task) =>
        task.id === id ? { ...task, state: !task.state } : task
      );

      return { ...list, list: updatedTasks };
    });
    setProject({ ...project, list: updatedLists as TaskList[] } as Project);
    // setTaskList(updatedLists as TaskList[]);
  };

  const handleDragOver = (
    targetId: string | null,
    type: "list" | "task" | null
  ) => {
    switch (type) {
      case "list":
        setHoverListId(targetId);
        return;
      case "task":
        setHoverTaskId(targetId);
        return;
      default:
        return;
    }
  };

  const handleDrop = () => {
    if (!draggingId || dragType === null || !project) return;

    const updatedProject = { ...project };

    if (dragType === "list" && hoverListId && draggingId !== hoverListId) {
      const fromIndex = updatedProject.list.findIndex(
        (l) => l.id === draggingId
      );
      const toIndex = updatedProject.list.findIndex(
        (l) => l.id === hoverListId
      );

      const [movedList] = updatedProject.list.splice(fromIndex, 1);
      updatedProject.list.splice(toIndex, 0, movedList);
    }

    if (
      dragType === "task" &&
      (hoverTaskId || hoverListId) &&
      draggingId !== hoverTaskId
    ) {
      let fromListIndex = -1;
      let toListIndex = -1;
      let fromTaskIndex = -1;
      let toTaskIndex = -1;

      updatedProject.list.forEach((taskList, listIdx) => {
        taskList.list?.forEach((task, taskIdx) => {
          if (task.id === draggingId) {
            fromListIndex = listIdx;
            fromTaskIndex = taskIdx;
          }
          if (task.id === hoverTaskId) {
            toListIndex = listIdx;
            toTaskIndex = taskIdx;
          }
        });
        if (hoverTaskId === null && taskList.id === hoverListId) {
          toListIndex = listIdx;
          toTaskIndex =
            taskList.list.length - (taskList.id === hoverListId ? 0 : 1);
        }
      });

      // console.log(hoverListId, hoverTaskId);

      // console.log(fromListIndex, toListIndex, fromTaskIndex, toTaskIndex);
      if (fromListIndex !== -1 && toListIndex !== -1) {
        const [movedTask] = updatedProject.list[fromListIndex].list.splice(
          fromTaskIndex,
          1
        );
        updatedProject.list[toListIndex].list.splice(toTaskIndex, 0, movedTask);
      }
    }

    setDragType(null);
    setProject(updatedProject);
    // setTaskList(updatedProject.list);
    setDraggingId(null);
    setHoverTaskId(null);
    setHoverListId(null);
  };

  useEffect(() => {
    const onMouseLeave = (e: MouseEvent) => {
      if (!e.relatedTarget && draggingId) {
        console.log("Mouse left the window");
        handleDrop();
      }
    };

    const onDragLeave = (e: DragEvent) => {
      if (!e.relatedTarget && draggingId) {
        console.log("Dragging left the body");
        handleDrop();
      }
    };

    const onDrop = (e: DragEvent) => {
      e.preventDefault();
      handleDrop();
    };

    const onDragOver = (e: DragEvent) => {
      e.preventDefault();
    };

    const onDragEnd = () => {
      handleDrop();
    };

    document.addEventListener("mouseleave", onMouseLeave);
    document.body.addEventListener("dragleave", onDragLeave);
    window.addEventListener("drop", onDrop);
    window.addEventListener("dragover", onDragOver);
    window.addEventListener("dragend", onDragEnd);

    return () => {
      document.removeEventListener("mouseleave", onMouseLeave);
      document.body.removeEventListener("dragleave", onDragLeave);
      window.removeEventListener("drop", onDrop);
      window.removeEventListener("dragover", onDragOver);
      window.removeEventListener("dragend", onDragEnd);
    };
  }, [draggingId]);

  useEffect(() => {
    const pj = createMockProject(id);
    setProject(pj);
  }, [id]);

  useEffect(() => {
    setHoverTaskId(null);
  }, [hoverListId]);

  return (
    <DNDContext.Provider
      value={{
        draggingId,
        hoverTaskId,
        hoverListId,
        dragType,
        handleDragStart,
        handleDragOver,
        handleMarkComplete,
        handleAddTask,
        handleDeleteList,
      }}
    >
      <div
        // onDragEnd={() => {
        //   handleDrop(); // always called
        // }}
        onDrop={(e) => {
          e.preventDefault(); // still needed if drop *does* happen
          handleDrop();
        }}
        onDragOver={(e) => e.preventDefault()} // required to allow drops
        className="task-table-container"
      >
        <div className="task-table-control">
          <div className="flex items-center gap-2">
            <span className="font-mono text-lg px-4">{project?.name}</span>
            <button
              className="button-3"
              onClick={() => setExpand((prev) => !prev)}
            >
              {expand ? <TableProperties /> : <ChartNoAxesColumn />}
            </button>
          </div>
          <div className="ml-auto flex flex-row gap-2 items-center justify-end">
            <button>
              <ListFilter />
            </button>
            <button className="button-2">+ Add Member</button>
          </div>
        </div>
        <div className="task-table grow">
          {project?.list?.map((tl) => (
            <TaskListComponent key={tl.id} list={tl} expand={expand} />
          ))}
          {isAdding ? (
            <div className="flex flex-col gap-2 p-2 rounded-md bg-black">
              <input
                ref={inputRef}
                required
                type="text"
                placeholder="List name"
                className="input-box text-sm"
              />
              <div className="text-sm flex flex-row gap-1">
                <button className="button-3" onClick={handleAddList}>
                  Add list
                </button>
                <button className="button-2" onClick={() => setIsAdding(false)}>
                  <X size={14} />
                </button>
              </div>
            </div>
          ) : (
            <button
              onClick={() => setIsAdding(true)}
              className="p-2 rounded-md bg-slate-400/30 backdrop-blur-sm hover:bg-slate-400/60 flex flex-row items-center gap-2 font-semibold text-sm "
            >
              {" "}
              <Plus /> Add new list
            </button>
          )}
        </div>
      </div>
    </DNDContext.Provider>
  );
}
