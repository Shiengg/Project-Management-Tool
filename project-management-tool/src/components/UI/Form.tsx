...
export default function Form({
  ...
  const modal = (
    <div className="fixed inset-0 z-50 w-screen h-screen bg-black/50 backdrop-blur-sm flex items-center justify-center">
      <div
        className="bg-black/70 rounded-lg max-w-[800px] w-full  h-fit max-h-screen p-2 flex flex-col gap-2 text-white"
        onMouseDown={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-center gap-2">
          <span className="font-mono text-xl text-gray-400">{name}</span>
